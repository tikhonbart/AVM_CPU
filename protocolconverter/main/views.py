...
def write_GACSECTOR():
    path = 'C:/Users/igor_/AVM_CPU/parser/res/xsystem.xml'
    tree = ET.parse(path)
    root = tree.getroot()
                                ...
                                if group5.findall('Struct'):
                                    for group6 in group5.findall('Struct'):
                                        for group7 in group6:
                                            ...
                                            tree.write('C:/Users/igor_/AVM_CPU/parser/res/xsystem_1.xml')
        ...
        IGNORE_LIST = ['Sector', 'Registers', 'Read_registers', 'Write_registers', 'Endpoints', 'Registers_type',
                                     'Start', 'Data_Type', 'Count', 'Bytes_Order', 'Period', 'Filter', 'Name', 'Composition', 'Control',
                                       'Description', 'Data', 'Command', 'Files', 'Read registers', 'Write registers',
                                        ...
                                        'Interrogation_periods'] # - динамические элементы
...
from main.models import IEC_60870_5_104_Master, IEC_60870_5_104_Slave, ModbusikMasterTCP, ModbusikMasterRTU, ModbusSlaveTCP, \
    ModbusSlaveRTU
import xml.etree.ElementTree as ET
...
def read_GACSECTOR(root, result):
    # IGNORE_LIST = ['Registers', 'Read_registers', 'Write_registers', 'Endpoints', 'Registers_type',
    #               'Start', 'Data_Type', 'Count', 'Bytes_Order', 'Period', 'Filter', 'Name', 'Composition', 'Control',
    #                'Description', 'Data', 'Command', 'Files'] # - динамические элементы
    for group1 in root.findall('GACSECTOR'):
        for group2 in group1:
            '''Ищем root'''
            if len(group2.attrib) > 0 and group2.attrib['name'] == 'root':
...
def get_data_from_database():
    ...
    for i in range(len(IEC_Master)):
        form1[IEC_Master[i].Name_ID] = {
            'Name_ID': IEC_Master[i].Name_ID, 'Sector': IEC_Master[i].Sector, 'IP':IEC_Master[i].IP, 'Port':IEC_Master[i].Port
            , 'Reconnect':IEC_Master[i].Reconnect, 'T1':IEC_Master[i].T1, 'T2':IEC_Master[i].T2, 'T3':IEC_Master[i].T3,
            'W':IEC_Master[i].W, 'K':IEC_Master[i].K, 'TimeSync':IEC_Master[i].TimeSync, 'TimeSyncFormat':IEC_Master[i].TimeSyncFormat,
            'TimeShift':IEC_Master[i].TimeShift, 'Filter':IEC_Master[i].Filter, 'NumberOfReconnections':IEC_Master[i].NumberOfReconnections,
            'SplitIntoParts':IEC_Master[i].SplitIntoParts, 'LogLevel':IEC_Master[i].LogLevel, 'Autorun':IEC_Master[i].Autorun,
            'Data':IEC_Master[i].Data, 'Command':IEC_Master[i].Command, 'Files':IEC_Master[i].Files, 'Endpoints':IEC_Master[i].Endpoints,
    ...
    # получаем список полей модели
        ...
        # переписываем код с IEC_Master на IEC_Slave и с другими полями
            ...
            'Name_ID': IEC_Slave[i].Name_ID, 'Sector': IEC_Slave[i].Sector, 'Port': IEC_Slave[i].Port, 'Name': IEC_Slave[
                i].Name, 'ASDU': IEC_Slave[i].ASDU, 'Address': IEC_Slave[i].Address, 'Count': IEC_Slave[i].Count, 'Type':
            IEC_Slave[i].Type,
            'Groups': IEC_Slave[i].Groups, 'Cause_of_transmission': IEC_Slave[i].Cause_of_transmission, 'IOA': IEC_Slave[
                i].IOA, 'Range': IEC_Slave[i].Range, 'Type_send': IEC_Slave[i].Type_send, 'AnswerTimeout': IEC_Slave[
                i].AnswerTimeout,
            'InfoTimeout': IEC_Slave[i].InfoTimeout, 'Downtime': IEC_Slave[i].Downtime, 'W': IEC_Slave[i].W, 'K': IEC_Slave[
                i].K, 'MaxConnectionCount': IEC_Slave[i].MaxConnectionCount, 'QueueSize': IEC_Slave[i].QueueSize, 'Filter':
            IEC_Slave[i].Filter,
            'LogLevel': IEC_Slave[i].LogLevel, 'Autorun': IEC_Slave[i].Autorun, 'Description': IEC_Slave[
                i].Description, 'Data': IEC_Slave[i].Data, 'Command': IEC_Slave[i].Command, 'Files': IEC_Slave[i].Files
            }
    #
    for i in range(len(ModMasterRTU)):
        form3[ModMasterRTU[i].Name_ID] = {
                'Name_ID': ModMasterRTU[i].Name_ID, 'Sector': ModMasterRTU[i].Sector, 'Port':ModMasterRTU[i].Port, 'Speed':ModMasterRTU[i].Speed,            'Parity':ModMasterRTU[i].Parity, 'Stop_bits':ModMasterRTU[i].Stop_bits, 'Data_bits':ModMasterRTU[i].Data_bits,
                'Number_of_retries':ModMasterRTU[i].Number_of_retries, 'Read_timeout':ModMasterRTU[i].Read_timeout, 'Pause':ModMasterRTU[i].Pause,            'Station_Address':ModMasterRTU[i].Station_Address, 'Reconnect':ModMasterRTU[i].Reconnect, 'Allowed_errors':ModMasterRTU[i].Allowed_errors,
                'LogLevel':ModMasterRTU[i].LogLevel, 'Autorun':ModMasterRTU[i].Autorun,'Registers_Type':ModMasterRTU[i].Registers_Type,            'Start':ModMasterRTU[i].Start, 'Data_Type':ModMasterRTU[i].Data_Type, 'Count':ModMasterRTU[i].Count,
                'Bytes_Order':ModMasterRTU[i].Bytes_Order, 'Period':ModMasterRTU[i].Period, 'Filter':ModMasterRTU[i].Filter,            'Name':ModMasterRTU[i].Name, 'Composition':ModMasterRTU[i].Composition, 'Control':ModMasterRTU[i].Control,
